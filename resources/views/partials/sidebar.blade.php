@php
    $sidebarRoutes = \App\Models\Route::orderBy('nume')->get();
@endphp

<aside class="sidebar" id="sidebar">

    <div class="sidebar-header" id="sidebarHeader">
        <span class="sidebar-brand" id="sidebarBrand">Avicola-Teovera</span>
        <span class="sidebar-sub sidebar-user-name" id="sidebarUser">{{ auth()->user()->name }}</span>
    </div>

    <nav class="sidebar-nav" id="sidebarNav">

        {{-- PRINCIPAL --}}
        <div class="sidebar-section sidebar-section--principal">{{ __('admin.nav_principal') }}</div>

        {{-- Toate comenzile --}}
        <a href="{{ route('orders.index') }}"
           class="sidebar-link sidebar-link--comenzi {{ request()->routeIs('orders.*') && !request('route_id') ? 'active' : '' }}"
           id="sidebarLinkComenzi">
        ...
        <a href="{{ route('orders.index', ['route_id' => $sidebarRoute->id]) }}"
           class="sidebar-sub-link sidebar-sub-link--ruta {{ request('route_id') == $sidebarRoute->id ? 'active' : '' }}"
           id="sidebarRuta{{ $sidebarRoute->id }}"
           title="{{ $sidebarRoute->nume }}">
        ...
        @if(auth()->user()->isAdmin())
            {{-- CATALOG --}}
            <div class="sidebar-section sidebar-section--catalog">{{ __('admin.nav_catalog') }}</div>

            <a href="{{ route('products.index') }}"
               class="sidebar-link sidebar-link--produse {{ request()->routeIs('products.*') ? 'active' : '' }}"
               id="sidebarLinkProduse">
            ...
            <a href="{{ route('categories.index') }}"
               class="sidebar-link sidebar-link--categorii {{ request()->routeIs('categories.*') ? 'active' : '' }}"
               id="sidebarLinkCategorii">
            ...
            {{-- ADMINISTRARE --}}
            <div class="sidebar-section sidebar-section--administrare">{{ __('admin.nav_administrare') }}</div>

            <a href="{{ route('routes.index') }}"
               class="sidebar-link sidebar-link--rute {{ request()->routeIs('routes.*') ? 'active' : '' }}"
               id="sidebarLinkRute">
            ...
            <a href="{{ route('stores.index') }}"
               class="sidebar-link sidebar-link--magazine {{ request()->routeIs('stores.*') ? 'active' : '' }}"
               id="sidebarLinkMagazine">
            ...
            <a href="{{ route('users.index') }}"
               class="sidebar-link sidebar-link--utilizatori {{ request()->routeIs('users.*') ? 'active' : '' }}"
               id="sidebarLinkUtilizatori">
