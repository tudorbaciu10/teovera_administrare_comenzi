...
<head>
    <meta charset="UTF-8">
    <title>Comandă {{ $order->store->denumire }} {{ $order->data->format('d.m.Y') }}</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media print {
            ...
            .page { margin: 0; padding: 12mm 12mm 8mm; }
        }
...
<div class="no-print" style="background:#1f2937;padding:10px 20px;display:flex;align-items:center;gap:16px;">
    ...
    <span style="color:#9ca3af;font-size:13px;">{{ $order->store->denumire }} · {{ $order->data->format('d.m.Y') }}</span>
...
<div class="page">
    ...
    <div class="info-grid">
        ...
        <div><span class="label">Ora trimiterii:</span></div>
        <div><span class="value">{{ $order->created_at->format('d.m.Y H:i') }}</span></div>

        <div><span class="label">Status:</span></div>
        <div><span class="value" style="text-transform:uppercase;color:#c0392b;">
            {{ match($order->status) { 'trimisa' => 'Trimisă', 'printata' => 'Printată', 'livrata' => 'Livrată', default => $order->status } }}
        </span></div>
    </div>

    {{-- Tabel produse --}}
    @php
        $items = $order->items->sortBy('product.category.ordine_sortare');
    ...
    <table>
        ...
        <tbody>
            ...
            @endforeach
            <tr class="total-row">
                <td colspan="3">Total produse comandate</td>
                <td>{{ $totalItems }} {{ $totalItems === 1 ? 'articol' : 'articole' }}</td>
            </tr>
        </tbody>
    </table>
    ...
    @if ($order->observatii)
    <div class="obs-box">
        <strong>Observații:</strong> {{ $order->observatii }}
    </div>
    @endif
    ...
    {{-- Footer --}}
    <div class="footer">
        <span>Generat automat · Avicola-Teovera</span>
