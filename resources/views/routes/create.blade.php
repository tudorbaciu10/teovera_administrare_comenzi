...
@section('title', __('admin.btn_creeaza').' — '.__('admin.rute_titlu'))
@section('page-title', __('admin.rute_titlu'))

@section('content')
<div class="card" style="max-width:600px">
    <div class="card-header">
        <span class="card-title">{{ __('admin.btn_creeaza') }} rută</span>
            ...
            <div class="form-group {{ $errors->has('zile_livrare') ? 'has-error' : '' }}">
                ...
                <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:4px">
                    ...
                    <label style="font-weight:400;display:flex;align-items:center;gap:4px;cursor:pointer">
                        <input type="checkbox" name="zile_livrare[]" value="{{ $zi }}"
                               {{ in_array($zi, old('zile_livrare', [])) ? 'checked' : '' }}>
                        {{ ucfirst($zi) }}
                    </label>
                    @endforeach
                </div>
            ...
            <div class="form-row">
                <div class="form-group {{ $errors->has('zi_cutoff') ? 'has-error' : '' }}">
                    ...
                    <select name="zi_cutoff">
                        ...
                        @foreach($zile as $zi)
                            <option value="{{ $zi }}" {{ old('zi_cutoff') === $zi ? 'selected' : '' }}>{{ ucfirst($zi) }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group {{ $errors->has('ora_cutoff') ? 'has-error' : '' }}">
                    ...
                    <input type="time" name="ora_cutoff" value="{{ old('ora_cutoff', '18:00') }}">
