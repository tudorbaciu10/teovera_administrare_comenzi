...
@section('title', __('admin.btn_editeaza').' — '.__('admin.produse_titlu'))
@section('page-title', __('admin.produse_titlu'))

@section('content')
<div class="card" style="max-width:600px">
    <div class="card-header">
        <span class="card-title">{{ __('admin.btn_editeaza') }}: {{ $product->nume_ro }}</span>
        ...
        <form method="POST" action="{{ route('products.update', $product) }}">
            ...
            <div class="form-row">
                <div class="form-group {{ $errors->has('nume_ro') ? 'has-error' : '' }}">
                    ...
                    <input type="text" name="nume_ro" value="{{ old('nume_ro', $product->nume_ro) }}">
                ...
                <div class="form-group {{ $errors->has('nume_ru') ? 'has-error' : '' }}">
                    ...
                    <input type="text" name="nume_ru" value="{{ old('nume_ru', $product->nume_ru) }}">
            ...
            <div class="form-row">
                <div class="form-group {{ $errors->has('category_id') ? 'has-error' : '' }}">
                    ...
                    <select name="category_id">
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ old('category_id', $product->category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->nume_ro }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group {{ $errors->has('unitate') ? 'has-error' : '' }}">
                    ...
                    <select name="unitate">
                        <option value="kg"  {{ old('unitate', $product->unitate) === 'kg'  ? 'selected' : '' }}>kg</option>
                        <option value="buc" {{ old('unitate', $product->unitate) === 'buc' ? 'selected' : '' }}>buc</option>
                    </select>
            ...
            <div class="form-group">
                <label class="flex items-center gap-2" style="font-weight:400;cursor:pointer">
                    <input type="checkbox" name="activ" value="1" {{ old('activ', $product->activ) ? 'checked' : '' }}>
                    {{ __('admin.col_activ') }}
                </label>
            </div>
